 ...
 * @author Carlos Cotom
 */
public class ReporteRevistasComentadasController {
    
    private final ReporteAdminDB dataReporte = new ReporteAdminDB();
    private final ReporteRevistaComentada filtroReporte;
    private boolean hayFechas;

    public ReporteRevistasComentadasController(ReporteRevistaComentada filtroReporte) {
    ...
    public ArrayList<ContenidoReporteRevistaComentada> getContenidoReporte() {
        ArrayList<ContenidoReporteRevistaComentada> contenido;
        //  Eceneario 1: Las Fechas fueron ingresadas
        //  Eceneario 2: No se ingreso ningun dato
        String query = this.generarConsultaWHERE();
        contenido = this.dataReporte.getReporteRevistasComentadas(query, this.contarIterrogantes(query), this.filtroReporte);
        return contenido;
