 ...
 * @author Carlos Cotom
 */
public class ReporteSuscripcionesController {
    
    private final ReporteEditorDB dataReporte = new ReporteEditorDB();
    private final ReporteSuscripcion filtroReporte;
    private boolean hayFechas;
    private boolean hayIdRevista;

    public ReporteSuscripcionesController(ReporteSuscripcion filtroReporte) {
    ...
    public ArrayList<ContenidoReporteSuscripcion> getContenidoReporte() {
        if (!this.filtroReporte.getIdRevista().equals("")) {
            this.hayIdRevista = true;
        }
        ArrayList<ContenidoReporteSuscripcion> contenido;
        if (hayFechas || hayIdRevista) {
            ...
            contenido = this.dataReporte.getReporteSuscripciones(query, this.contarIterrogantes(query), this.filtroReporte);
            ...
            contenido = this.dataReporte.getFullReporteSuscripciones(idsRevistaEditor);
            this.generarNumeroRevista(contenido);
            return contenido;
        }
    }
    
    private void generarNumeroRevista(ArrayList<ContenidoReporteSuscripcion> contenido) {
        int numero = 1;
        int indice = 0;
        String nombreRevistaActual;
        String nombreRevistaAnterior = "";
        for (ContenidoReporteSuscripcion reporte : contenido) {
